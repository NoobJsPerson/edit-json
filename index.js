...
function applyrelative(file, dirname, relative){
	return dirname && relative && ? path.join(dirname,file) : file;
}
...
module.exports = class EditClient {
	 ...
	 */
	constructor({ defaultFile, dirname, relative, autosave, parseFirst, placeholderObj = {} } = {}) {
		defaultFile = applyrelative(defaultFile, dirname, relative)
		this = {
			...this,
			relative,
			dirname,
			...
			defaultFile,
			placeholderObj,
			currentObj: fetchFirst ? JSON.parse(fs.readFileSync(defaultFile)) : placeholderObj
		}
	}
 ...
 if (!file) throw ReferenceError("No JSON to edit");
	...
	setSync(property, value, file = this.defaultFile, relative) {
		if (!file) throw ReferenceError("No JSON to edit");
		file = applyrelative(file, this.dirname, relative)
		if(this.file != this.defaultFile || this.currentObj == this.placeholderObj){
